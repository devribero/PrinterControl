...
## Corpus Check
- 135 files · ~87,434 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1213 nodes · 2516 edges · 74 communities (64 shown, 10 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 120 edges (avg confidence: 0.52)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `566eee97`
...
Cohesion: 0.13
Nodes (19): FORM_VAZIO, formatarData(), FormState, UsersView(), abrirEdicao(), confirmarAtivacao(), salvar(), validar() (+11 more)
...
### Community 25 - "Alert"
Cohesion: 0.15
Nodes (18): Alert, SQLModel, TonerHistory, get_alert(), list_alerts(), notify_alert(), get, patch (+10 more)
...
Cohesion: 0.14
Nodes (18): AlertsDonutCard(), AlertsDonutCardProps, BottomChartsProps, PagesConsumedCard(), TotalPrintsCard(), MonthlyCountersProps, NavItem(), NavItemProps (+10 more)
...
### Community 31 - "tests_webhook.py"
Cohesion: 0.13
Nodes (20): Alert, PrinterReading, _active(), evaluate_reading(), Session, Alertas automaticos (Etapa 8A). Roda logo apos cada PrinterReading ser…, Cria, mantem ou resolve o alerta de uma condicao. Retorna a acao tomada., Avalia uma leitura e sincroniza os alertas da impressora. Faz commit ao final.… (+12 more)
...
### Community 33 - "cn"
Cohesion: 0.16
Nodes (15): DashboardPage(), AlertBanner(), AlertBannerProps, AlertsView(), AlertsViewProps, BottomCharts(), MonthlyCounters(), StatCard() (+7 more)

### Community 34 - "Role"
Cohesion: 0.09
Nodes (34): SQLModel, str, RBAC minimo (Fase 1). Tres papeis cobrem todas as rotas existentes hoje: -…, Role, login(), post, Session, _active_admin_count() (+26 more)
...
Cohesion: 0.10
Nodes (23): react, AlertsPage(), HistoryPage(), PrintersPage(), ReportsPage(), TonerPage(), AppShell(), AuthGate() (+15 more)
...
Cohesion: 0.12
Nodes (16): PrinterDetailsModal(), PrinterDetailsModalProps, PrinterTable(), RightPanel(), classify(), FILTERS, SummaryCard(), SummaryCardProps (+8 more)

### Community 43 - "dependencies.py"
Cohesion: 0.22
Nodes (9): get_session(), Session, Dependencias compartilhadas pelas rotas. Autorizacao (Fase 1) fica CENTRALIZADA…, Usuario dono do JWT do header Authorization. 401 se ausente/invalido. O usuario…, Fabrica de dependencia: exige que o usuario tenha (ou herde) um dos papeis…, require_roles(), require_user(), decode_token() (+1 more)
...
Cohesion: 0.13
Nodes (12): ibmPlexMono, metadata, publicSans, sourceSerif, Providers(), ToastContext, ToastContextValue, ToastItem (+4 more)
...
Cohesion: 0.26
Nodes (11): config, PrinterStatusBadge(), PAGE_SIZE_OPTIONS, PrinterTableProps, DEFAULT_FILTERS, filterPrinters(), PrinterFilters, getPrinterType() (+3 more)

### Community 57 - "printers.ts"
Cohesion: 0.14
Nodes (14): Levantamento_impressões (planilha original), DecommissionedList(), DecommissionedListProps, RightPanelProps, DecommissionedPrinter, decommissionedPrinters, globalToner, monthlyUsage (+6 more)

### Community 58 - "auth.ts"
Cohesion: 0.12
Nodes (23): ACTIVE_NODES, features, Login(), handleSubmit(), LoginProps, NETWORK_LINKS, NETWORK_NODES, ApiError (+15 more)
...
Cohesion: 0.26
Nodes (11): check(), check_true(), _confere_rbac_do_frontend(), h(), main(), Fase 1 - Autenticacao, RBAC e protecao das rotas. Diferente dos demais…, O frontend tem a sua propria copia da hierarquia de papeis, em…, Cria um banco no formato ANTERIOR a Fase 1 (users sem role/is_active), roda a… (+3 more)
...
Cohesion: 0.15
Nodes (12): PrinterMonthly, SQLModel, collect_fleet(), Etapa 11 - coleta simulada da frota inteira, ponta a ponta. Copia o banco real…, Uma coleta completa. Devolve resumo por status., check(), check_true(), h() (+4 more)
...
### Community 71 - "tests_uptime.py"
Cohesion: 0.17
Nodes (13): AsyncIOScheduler, health_check(), lifespan(), get, read_root(), Coleta agendada (Etapa 7; frota inteira desde a Etapa 5). APScheduler roda…, Um ciclo de coleta: toda a frota ativa, agrupada por IP…, Liga o scheduler conforme o .env. Retorna None quando desabilitado. (+5 more)

### Community 72 - "User"
Cohesion: 0.18
Nodes (17): True se o papel do usuario satisfaz qualquer um dos exigidos., User, get, Conta autenticada e seu papel — usado para decidir o que exibir/permitir., read_current_user(), hash_password(), verify_password(), Cria as contas de teste e uma impressora + alerta para as rotas de escrita. (+9 more)
...
## Knowledge Gaps
- **260 isolated node(s):** `$schema`, `typescript`, `oxc`, `react/rules-of-hooks`, `warn` (+255 more)
...
- **Why does `SNMPResult` connect `SNMPResult` to `enrich_discovered_printers`, `PrinterCollector`, `tests_printer_fleet.py`, `SNMPClient`, `printer_collector.py`, `snmp_fleet_mock.py`, `discovery.py`?**
  _High betweenness centrality (0.042) - this node is a cross-community bridge._
- **Why does `Printer` connect `Printer` to `models/printer.py`, `PrinterCollector`, `create_server`, `collect.py`, `tests_uptime.py`, `tests_printer_fleet.py`, `User`, `printer_sync.py`, `servers.py`, `printer_collector.py`, `Alert`, `tests_rbac.py`, `database.py`, `tests_webhook.py`?**
